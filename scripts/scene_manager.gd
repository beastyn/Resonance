...
@export var finish_light:Light2D
@export var finish_transition_duration: float = 0.5
...
func _ready() -> void:
	...
	ResonanceSignals.destroyed_by_resonance.connect(_on_destroyed_by_resonance)
...
func _on_stop_listening_wave(wave_area: WaveArea) -> void:
	if !_canvas_color_change_tween.is_valid(): 
		...
		_canvas_color_change_tween = create_tween()
	if _canvas_color_change_tween.is_running(): _canvas_color_change_tween.stop()
	_canvas_color_change_tween.play()
...
func _on_cutscene_trigger_body_entered(body: Node2D) -> void:
	if body.is_in_group("player"):
		emit_signal("need_start_cutscene")			

func _on_enemy_trigger_body_entered(body: Node2D) -> void:
	if body.is_in_group("player"):
		emit_signal("need_enemy") # Replace with function body.

func _on_destroyed_by_resonance(groups: Array[StringName]) -> void:
	if groups.has("finish_trigger"):
		finish_light.visible = true

func _on_finish_body_entered(body: Node2D) -> void:
	if body.is_in_group("player"):
		TransitionManager.transite_to("res://scenes/outro.tscn", finish_transition_duration)
