...
func _solve_inputs(delta: float) -> void:	
	...
	# Handle jump.
	if Input.is_action_just_pressed("Jump") and is_on_floor() and !_is_attracted:
		velocity_component.apply_jump_veloctiy()		

	# Get the input direction and handle the movement/deceleration.
	direction = Input.get_axis("move_left", "move_right")
	velocity_component.apply_in_direction(direction, delta)
...
func _solve_animations() -> void:
	animation_component.set_movement_direction(direction)
