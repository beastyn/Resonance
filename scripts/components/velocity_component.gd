...
func apply_jump_veloctiy(strength: float = 0.0) -> void:
	_velocity.y = -(jump_strength if strength == 0.0 else strength)
		
func apply_in_horizontal_direction(direction: float, delta: float) -> void:
...
func apply_in_vertical_direction(direction: float, delta: float) -> void:
	_acceleration_time += delta
	if direction: 
		_apply_vertical_acceleration(direction, speed, acceleration_curve, _acceleration_time, delta)		
	#elif _velocity.y != 0.0:
		#_velocity.y = lerpf(_velocity.y, 0.0, decelerationSpeed * delta)
...
func apply_attraction(delta: float, strength: float, direction: Vector2, attraction_curve: Curve) -> void:
	...
	_apply_fly_acceleration_in_direction(-Vector2(direction.x,0), strength, attraction_curve, _attraction_time, delta)
	_is_attracted = true
...
func _apply_vertical_acceleration(direction: float, max_speed:float, increase_curve: Curve, increase_time: float, delta:float) -> void:
	#var multiplyer := increase_curve.sample(clamp(increase_time, 0.0, 1.0))
	_velocity.y = direction * max_speed
...
func _apply_fly_acceleration_in_direction(direction: Vector2, max_speed:float, increase_curve: Curve, increase_time: float, delta:float) -> void:
	var calc_dir = Vector2(1.0*direction.x/abs(direction.x), -0.5)
	var multiplyer := increase_curve.sample(clamp(increase_time, 0.0, 1.0))
